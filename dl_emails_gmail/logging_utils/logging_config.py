...
This module provides centralized logging configuration for any Python project.
...
- get_script_dir(): Get script directory for frozen/normal execution
...
LOGGING_CONFIG = {
    "defaults": {
        ...
        "log_dir": None  # resolved to SCRIPT_DIR/logs via determine_log_dir()
    },
    "loggers": {
        # Example per-logger overrides:
        # "dl_gmail": {
        #     "file_level": "DEBUG",
...
def get_script_dir() -> Path:
    """
    Get the script directory, handling both frozen (PyInstaller) and normal execution.
    
    Returns:
        Path: The directory containing the main script or executable
    """
...
def determine_log_dir(base_dir: Optional[Path] = None) -> Path:
    ...
    """
    if base_dir is None:
        return get_script_dir() / DEFAULT_LOG_DIR
...
def get_logger(
    ...
    Example:
        >>> logger = get_logger('dl_gmail', console_level='DEBUG')
