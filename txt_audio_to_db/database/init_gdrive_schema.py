...
This script initializes the GDrive database schema by executing the gdrive_schema.sql DDL script.
It can be run multiple times safely as all CREATE statements use IF NOT EXISTS.

Usage:
    python database/init_gdrive_schema.py
    
Features:
- Executes gdrive_schema.sql against the configured database
...
from txt_audio_to_db.config.db_config import DB_CONFIG, get_db_config
from common.logging_utils.logging_config import get_logger

# Initialize logger for this module
logger = get_logger("init_gdrive_db")
...
def execute_sql_file(connection, sql_file_path: Path) -> bool:
    ...
    Example:
        >>> success = execute_sql_file(conn, Path("gdrive_schema.sql"))
        >>> if success:
...
def verify_tables_exist(connection) -> bool:
    ...
    expected_tables = [
        'gdr_diary',
        'gdr_source_file', 
        'gdr_transcription_run',
        'gdr_transcription_usage',
        'gdr_schema_versions'
    ]
...
def main():
    ...
    3. Executes the gdrive_schema.sql DDL script
    4. Verifies table creation
    5. Reports schema version
    """
    try:
        logger.info("Starting GDrive database initialization...")
        ...
        script_dir = Path(__file__).resolve().parent
        schema_file = script_dir / "gdrive_schema.sql"
        ...
        logger.info("Executing GDrive database schema...")
        with psycopg2.connect(**DB_CONFIG.get_connection_params()) as conn:
        ...
        logger.info("GDrive database initialization completed successfully!")
        print("\n" + "="*60)
        print("GDRIVE DATABASE INITIALIZATION SUCCESS")
        print("="*60)
        print("+ Database connection established")
        print("+ Schema file executed")
        print("+ All tables created")
        print("+ Schema version recorded")
        print("\nYou can now run the GDrive transcription system with database integration.")
        print("="*60)
        
        return True
        
    except Exception as e:
        logger.error(f"GDrive database initialization failed: {e}")
        print(f"\n- GDrive database initialization failed: {e}")
        return False
